...
def get_token(botName):
    ...
    logger.info('Obtido o token {} para o bot {}'.format(botToken, botName))
    return botToken
...
def subscribe_chat_to_topic(topic, chatId):
    ...
    logger.info('Resposta recebida para a inscrição: {}'.format(response))
...
def create_chat_sqs(queueName):
    ...
    logger.info(f"Criando a fila de nome {queueName}")
    ...
    if not queueExists:
        lambdaClient.create_event_source_mapping(
            ...
            FunctionName='broadcast-message'
        )
        #Inserir política que permite que SNSs publiquem na fila
            ...
            "Statement": [
                {
                    ...
                    "Resource": f"arn:aws:sqs:{awsRegion}:{awsAccount}:{queueName}",
                    "Condition": {
                        "StringEquals": {
        ...
        attributes = {
                'Policy':json.dumps(queueSendMessagePolicy)
            }
        logger.info("URL da fila é {}. Inserindo os atributos {}".format(queueUrl, attributes))
...
def broadcast_message(chatId, message, topic):
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    logger.info('Divulgando mensagem {} para o tópico {}, vinda do chat {}'.format(message, topic, chatId))
    # Confirma que o tópico já existe. Essa ação é idempotente
